...
export const CartProvider = ({ children }) => {
  ...
  const fetchCart = async () => {
    ...
    try {
      setCartLoading(true);
      const response = await fetch(`http://10.16.57.136:8000/api/cart/${encodeURIComponent(user.email)}`);
  ...
  const addToCart = async (donationId) => {
    ...
    try {
      const url = "http://10.16.57.136:8000/api/cart/add";
      const payload = {
        userId: user.email,
        donationId,
      };
  ...
  const removeFromCart = async (donationId) => {
    ...
    try {
      const response = await fetch("http://10.16.57.136:8000/api/cart/remove", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userId: user.email,
  ...
  const clearCart = async () => {
    ...
    try {
      const response = await fetch(`http://10.16.57.136:8000/api/cart/clear/${encodeURIComponent(user.email)}`, {
        method: "DELETE",
      });
