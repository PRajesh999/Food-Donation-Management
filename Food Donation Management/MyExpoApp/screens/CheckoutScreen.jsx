...
const foodTypeImages = {
  ...
  "Veg Curry": require("../assets/Veg Curry.jpg"),
  "Non Veg Curry": require("../assets/Non Veg Curry.jpg"),
  "Milk": require("../assets/Milk.jpg"),
  "Juice / Soft Drinks": require("../assets/Juice.jpg"),
  "Sweets": require("../assets/Sweet.jpg"),
};
...
export default function CheckoutScreen({ navigation }) {
  ...
  const handleClaim = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("http://10.16.57.136:8000/api/cart/checkout", {
        method: "POST",
  ...
  if (cartItems.length === 0) {
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text style={[styles.emptyText, { color: theme.colors.textSecondary }]}>Redirecting to cart...</Text>
      </View>
    );
  }
        ...
        {cartItems.filter(item => item.quantity > 0 && servings[item.donationId] > 0).map((item) => (
          <View key={item.donationId} style={[styles.itemCard, { backgroundColor: theme.colors.card }]}> 
            <View style={styles.itemRow}>
              <Image source={foodTypeImages[item.foodType] || foodTypeImages["Rice"]} style={styles.foodImage} />
...
const styles = StyleSheet.create({
  ...
  foodImage: { width: 64, height: 64, borderRadius: 12, backgroundColor: '#eee' },
