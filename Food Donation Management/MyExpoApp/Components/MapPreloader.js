  ...
  useEffect(() => {
    const preloadMapData = async () => {
      try {
        ...
        const donationsResponse = await axios.get("http://10.16.57.136:8000/api/donations/get-donations");
        ...
        if (user?.email) {
          const cartResponse = await fetch(`http://10.16.57.136:8000/api/cart/${encodeURIComponent(user.email)}`);
          if (cartResponse.ok) {
