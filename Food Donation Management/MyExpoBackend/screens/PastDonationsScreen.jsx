...
const foodTypeImages = {
  Veg: require("../assets/Veg Curry.jpg"),
  "Non Veg": require("../assets/Non Veg Curry.jpg"),
  Others: require("../assets/Food_On_Table.jpeg"),
};
...
export default function PastDonationsScreen({ navigation, route }) {
  ...
  const fetchDonations = useCallback(async (isRefreshing = false) => {
    ...
    try {
      const response = await fetch(`http://10.16.57.136:8000/api/donations/user/${encodeURIComponent(userEmail)}`);
  ...
  const debugAllDonations = async () => {
    try {
      ...
      const response = await axios.get("http://192.168.26.19:8000/api/donations/get-donations");
    ...
    return (
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'stretch', padding: 18 }}>
          ...
          <View style={{ alignItems: 'center', justifyContent: 'flex-start', minWidth: 110, backgroundColor: '#F7F7F7', borderRadius: 16, paddingVertical: 8, paddingHorizontal: 8, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 6, marginLeft: 8 }}>
            <Image
              source={foodTypeImages[item.foodType] || foodTypeImages["Rice"]}
              style={[
